	...
	private void initialize() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(e.getSource().equals(comboBoxDate))
				{
					if(comboBoxDate.getSelectedItem() != null)
					{
						...
						ProjectDriver.getTimeSched(doc2, date);
					}
				}
			}
		});
		...
		comboBoxDoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ProjectDriver.getDateSched(doc1);
			}
		});
		...
		btnRemoveFromQueue.addComponentListener(new ComponentAdapter() {
			...
			public void componentShown(ComponentEvent arg0) {
				if(ProjectDriver.patients.isEmpty())
				{
					lblAttentionApp.setVisible(false);
					lblPatientQueueApp.setVisible(false);
					btnRemoveFromQueue.setVisible(false);
				...
				{
					lblPatientQueueApp.setText(ProjectDriver.patients.peek().toString());
				}
			}
		});
		btnRemoveFromQueue.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				ProjectDriver.patients.dequeue();
				if(ProjectDriver.patients.isEmpty())
				{
					lblAttentionApp.setVisible(false);
					lblPatientQueueApp.setVisible(false);
		...
		btnSubmitA.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				ProjectDriver.apps.insert(app);
				ProjectDriver.appTable();
				...
				ProjectDriver.getDateSched(docIndex);
				if(!ProjectDriver.patients.isEmpty() && ProjectDriver.patients.peek().toString().equals(pat.toString()))
				{
					ProjectDriver.patients.dequeue();
					if(ProjectDriver.patients.isEmpty())
					{
		...
		btnCancelApp.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				Appointment appFind = ProjectDriver.apps.search(appCxl);
				if(appFind != null)
				{
					ProjectDriver.apps.delete(appFind);
					ProjectDriver.appTable();
				}
		...
		btnSubmitD.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				ProjectDriver.docs.insert(doc);
				ProjectDriver.docTable();
		...
		btnUpdateDocUp.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				Doctor docFind = ProjectDriver.docs.search(doc);
				if(docFind != null)
				{
		...
		btnSubmitP.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				ProjectDriver.pats.insert(pat);
				ProjectDriver.patients.enqueue(pat);
				ProjectDriver.patTable();
		...
		btnUpdatePatUp.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				Patient patFind = ProjectDriver.pats.search(patUp);
				if(patFind != null)
				{
